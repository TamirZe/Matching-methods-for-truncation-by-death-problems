...
################################################################
# new xi_PredTreatEffect_SA, with beta.a and beta.n, so the EM will be compatible with the gammas of DGM-multi ####
# xi_PredTreatEffect_SA_fortheDGM = function(Z, D, X, eta = 0,
...
#       if(Z[i]==1&D[i]==1) {
#         #posterior probabilities
#         prob.10 = ( exp(t(beta.a_old)%*%X[i, ]) ) / ( exp(t(beta.a_old)%*%X[i, ]) + (1 + eta) )
#         prob.c = 1 - prob.10
# 
#         AugData = rbind(AugData, c(1, X[i, ], prob.10))
#         AugData = rbind(AugData, c(2, X[i, ], prob.c))
...
#       if(Z[i]==1&D[i]==0) {
#         #posterior probabilities
#         prob.10 = ( exp(t(beta.a_old)%*%X[i, ]) ) / ( exp(t(beta.a_old)%*%X[i, ]) + (1 + eta)/eta*exp(t(beta.n_old)%*%X[i, ]) )
#         prob.n = 1 - prob.10
# 
#         AugData = rbind(AugData, c(1, X[i, ], prob.10))
#         AugData = rbind(AugData, c(3, X[i, ], prob.n))
...
#     fit = multinom(AugData[, 1] ~ AugData[, (3:(V+1))], weights = AugData[, (V+2)], trace = FALSE)
#     betas  = coef(fit)
#     beta.a = betas[1, ] #### TODO check 16.07.22
#     beta.n = betas[2, ]
# 
#     iter = iter + 1
...
#     PROB = matrix(0, N, 4)
#     for(i in 1:N) {
#       prob.a = exp(t(beta.a)%*%X[i, ]) * 1/(1  + eta)
#       prob.d = exp(t(beta.a)%*%X[i, ]) * eta/(1 + eta)
#       prob.c = 1
#       prob.n = exp(t(beta.n)%*%X[i, ])
#       sum = prob.c + prob.d + prob.a + prob.n
# 
