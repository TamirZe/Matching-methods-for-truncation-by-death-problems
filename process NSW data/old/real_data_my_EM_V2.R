...
# TODO LL from NSW_data script
data = data
data= data_boot
covariates = c("intercept", "age", "black", "nodegree", "married")
iterations = 15
function_my_EM_real_data = function(data, covariates, iterations = 12){
  #X_sub_cols = colnames(dat_em)[grep("X", colnames(dat_em))]
  X_sub_cols = covariates
  ...
  gamma_as_initial = rep(0, length(X_sub_cols))
  gamma_ns_initial = rep(0, length(X_sub_cols))
  ...
  for(i in 1:iterations){
    ...
    gamma_pro_t = rep(0, length(X_sub_cols))
    ...
    #d = dup_dat_em[1:100,]
    w_by_stratum = apply(subset(dat_em, select = c(p_as, p_pro, p_ns)), 1, 
                         function(x) cbind(x[x>0], which(x>0)))
    ...
    dup_dat_em$G_EM = ifelse(dup_dat_em$G_EM == 1, "as", 
                      ifelse(dup_dat_em$G_EM == 2, "pro", 
  ...
  data_with_PS = data.table(merge(x = data,
  ...
  x = as.matrix(subset(data_with_PS, select = covariates))
  PS_est = cbind(exp(x%*%last_coeff_as), exp(x%*%last_coeff_ns), 
                 exp(x%*%gamma_pro_t))
  ...
  return(list(dat_em, data_with_PS,
              last_coeff_as, last_coeff_ns, gamma_as_list, gamma_ns_list))
}
