...
simulate_data_function = function(seed_num=NULL, gamma_as, gamma_ns, gamma_pro, param_n,
                                  ...
                                  epsilon_1_GPI = 1, only_mean_x_bool=FALSE){
  ...
  if(misspec_PS == 1){
    ...
    betas_GPI_adj = as.matrix( data.frame(betas_GPI, c(5,2), c(2,1)) )
  ...
  end_time <- Sys.time()
  return(list(dt=dt, x_obs=x_obs, x_PS=x_PS, x_outcome=x, OBS_table=OBS_table, pi=pi, probs_mean=probs_mean))
}
...
simulate_data_run_EM_and_match = function(return_EM_PS = FALSE, index_set_of_params, gamma_as, gamma_ns, gamma_pro,
  ...
  for (i in 1:param_n_sim){
    ...
    EM_list = function_my_EM(data_for_EM, iterations, epsilon_EM, dim_x=dim_x)
    end_time2 <- Sys.time()
