...
index_EM_not_conv = 0; i_EM_not_conv = c() # check proprtion of iteration when EM has not converged
#while (i <= param_n_sim){
for (i in 1:param_n_sim){
  ...
  list_mean_by_g[[i]] = apply(list_data_for_EM_and_X$mean_by_g, 2, as.numeric) # x_obs
  x_obs = list_data_for_EM_and_X$x_obs
  x_PS = data.frame(list_data_for_EM_and_X$x_PS)
  x_outcome = data.frame(list_data_for_EM_and_X$x_outcome)
  pis = list_data_for_EM_and_X$pis
  vec_OBS_table = list_data_for_EM_and_X$vec_OBS_table 
