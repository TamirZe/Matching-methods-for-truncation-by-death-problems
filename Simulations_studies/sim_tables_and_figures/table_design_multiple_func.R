...
#arrange_mean_and_sd_per_set(one_set=lst_separate_sets[[l]], HL_and_naive=HL_and_naive, estimators_names=estimators_names)
...
#arrange_all_sets_est_se_sd(rep_FALSE_or_TRUE_data = lst_rep_F_T[[i]],HL_and_naive=HL_and_naive,estimators_names=estimators_names)
...
design_tables_one_dataset_ALL_estimators = function(data_set, estimators_names, mat_all_estimators, 
  ...
  # estimators_names = c("Crude", "HL", "BC", "BC caliper")
  tab_est = mat_all_estimators
  final_tables_naive_HL_ONE_dataset = 
    design_tables_one_dataset(tab_est, data_set, HL_and_naive="HL_and_naive", estimators_names=estimators_names)
  ...
  tab_est_WLS = arrange_LS_before_design_tables_one_dataset(subset(WLS_YESint_mat_regression_estimators, select = -grep("naive|sandwi", colnames(WLS_YESint_mat_regression_estimators))), 
      subset(WLS_NOint_mat_regression_estimators, select = -grep("naive|sandwi", colnames(WLS_NOint_mat_regression_estimators))))
...
TABLES_before_coverage_and_wout_naive_and_DING = function(data_set_names_vec, estimators_names=c("Crude", "HL")){
  
  lst_final_tables_ALL_est_ALL_dataset_with_gamma = lapply(1:length(data_set_names_vec), function(l){
    design_tables_one_dataset_ALL_estimators(data_set=data_set_names_vec[l], estimators_names, mat_all_estimators,
             OLS_YESint_mat_regression_estimators, OLS_NOint_mat_regression_estimators, 
             WLS_YESint_mat_regression_estimators, WLS_NOint_mat_regression_estimators)
  })
  data_set_names_vec
...
TABLES_add_coverage = function(data_set_names_vec, lst_final_tables_ALL_est_ALL_dataset, list_all_CI_temp){
  ...
  calculate_coverage_in_tables = function(mat_all_CI_one_param){
  ...
  list_all_CI_temp_with_coverage = lapply(1:length(list_all_CI_temp), function(l){
    calculate_coverage_in_tables(list_all_CI_temp[[l]])
  })
  ...
  change_est_names = function(df_coverage){
    ...
    df_coverage$newname[grep("*\\_BCclpr_inter$",df_coverage$newname)] = "BC caliper inter"
    df_coverage$newname[grep("*\\_BC_inter$",df_coverage$newname)] = "BC inter"
    df_coverage$newname[grep("*\\_BCclpr$",df_coverage$newname)] = "BC caliper"
    df_coverage$newname[grep("*\\_BC$",df_coverage$newname)] = "BC"
    df_coverage$newname[grepl("rep",df_coverage$newname) & grepl("._PS$",df_coverage$newname, ignore.case = FALSE)] = "PS Crude"
    df_coverage$newname[grepl("rep",df_coverage$newname) & grepl("._maha$",df_coverage$newname)] = "mahal Crude"
  ...
  for(i in 1:length(lst_final_tables_ALL_est_ALL_dataset)){
    ...
    levels = c("PS Crude", "mahal Crude", "Crude", "HL", "OLS", "OLS inter", "WLS", "WLS inter", 
                      "BC", "BC caliper", "BC inter", "BC caliper inter")
    temp$Matching_estimator = factor(temp$Matching_estimator, levels=levels)
    temp = arrange(temp, Set, Replacements, Matching_estimator)
...
TABLES_add_naive_and_ding = function(data_set_names_vec, lst_final_tables_ALL_est_ALL_dataset, naives_before_matching_coverage){
  ...
  naives_before_matching_coverage$Matching_estimator = mgsub(naives_before_matching_coverage$Matching_estimator, 
        c("Naive without matching", "Survivors naive without matching"), c("Naive", "Naive survivors"))
...
adjustments_for_final_tables = function(fin_tab){
  for(i in 1:length(fin_tab)){
    fin_tab[[i]] = filter(fin_tab[[i]], !Matching_estimator %in% c("PS Crude", "mahal Crude"))
    ...
    fin_tab[[i]] = filter(fin_tab[[i]], !(Replacements=="No" & Estimator %in% c("BC caliper", "BC", "BC inter", "BC caliper inter")))
    #TODO ADD N and k
    fin_tab[[i]] = data.table("Set & parameter"=fin_tab[[i]][,1], N = param_n, dim_x = dim_x - 1, fin_tab[[i]][,-1])
  }
  return(fin_tab)
}

adjustments_for_final_tables_crude_est = function(fin_tab){
  for(i in 1:length(fin_tab)){
    fin_tab[[i]] = filter(fin_tab[[i]], Matching_estimator %in% c("PS Crude", "mahal Crude", "Crude",
                        "OLS", "WLS", "DingLu MA"))
    fin_tab[[i]] = subset(fin_tab[[i]], select = -grep("my_MSE", colnames(fin_tab[[i]])))
    fin_tab[[i]]$Set = paste0(fin_tab[[i]]$Set, " ", fin_tab[[i]]$SACE)
    #colnames(fin_tab[[i]]) = gsub("Set","Set & parameter", colnames(fin_tab[[i]]))
    fin_tab[[i]] = subset(fin_tab[[i]], select = -grep("SACE", colnames(fin_tab[[i]])))
    est_names_to_order = c("PS Crude", "mahal Crude", "Crude", "OLS", "WLS")
    fin_tab[[i]]$Matching_estimator = mgsub(fin_tab[[i]]$Matching_estimator, 
          est_names_to_order, paste0(LETTERS[1:length(est_names_to_order)], "_", est_names_to_order))
    
    fin_tab[[i]]$Replacements[fin_tab[[i]]$Replacements==""] = "ZZZ"
    fin_tab[[i]] = arrange(fin_tab[[i]], Set ,Replacements, Matching_estimator)
    fin_tab[[i]]$Matching_estimator = mgsub(fin_tab[[i]]$Matching_estimator,
      paste0(LETTERS[1:length(est_names_to_order)], "_", est_names_to_order), est_names_to_order)
    fin_tab[[i]]$Replacements = gsub("ZZZ", "", fin_tab[[i]]$Replacements)
    colnames(fin_tab[[i]]) = gsub("_"," ", colnames(fin_tab[[i]]))
    colnames(fin_tab[[i]]) = 
      mgsub(colnames(fin_tab[[i]]), c("Set", "Matching estimator"),c("Set & parameter", "Estimator"))
...
#fin_tab_df = final_tables_general$`_S1`
adjustments_for_tables_before_present = function(fin_tab_df){
  colnames(fin_tab_df) = mgsub(colnames(fin_tab_df), c("dim_x", "Set & parameter"), c("k", "Set_parameter"))
  if(length(unique(fin_tab_df$Set_parameter)) == 3){
    fin_tab_df = filter(fin_tab_df, !substr(Set_parameter,1,1) == "A")
  }
  fin_tab_df = data.frame(fin_tab_df[,c(1:3)], 
                 Estimator = paste0(fin_tab_df$Estimator, " ", fin_tab_df$Replacements),
                 subset(fin_tab_df, select = c("Mean", "Emp SD", "Est SE", "MSE", "Coverage")))
  fin_tab_df$N = as.character(fin_tab_df$N); fin_tab_df$k = as.character(fin_tab_df$k)
  fin_tab_df$MSE = as.character(round(fin_tab_df$MSE, 3))
  fin_tab_df = subset(fin_tab_df, select = -c(N,k))
  return(fin_tab_df)
}
