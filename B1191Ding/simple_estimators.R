data = data_adjusted_all_subjects
...
some_simple_estimators = function(data){
  ...
  t_test_unemp_as_0 = t.test(treat$Y, control$Y, var.equal = F, paired = FALSE)

  # se_unemp_as_0 = sqrt( (sd(treat$Y))^2/ (nrow(treat)) +
  #             (sd(control$Y))^2 / (nrow(control)) )
  t_val_unemp_as_0 = qt(0.975, nrow(treat) + nrow(control) - 2, 
  ...
  t_test_surv = t.test(employed[A==1, Y], employed[A==0, Y], alternative = 
     "two.sided", mu = 0, paired = FALSE, var.equal = F, conf.level = 0.95)
  ...
  return(list(mean_trt = mean(treat$Y), mean_ctr = mean(control$Y),
              naive_est = naive_est, t_test_unemp_as_0 = t_test_unemp_as_0,
              surv_est = surv_est, t_test_surv = t_test_surv,
              GG_estimator = GG_estimator, GD_estimator = GD_estimator, 
              DD_estimator = DD_estimator, bounds_mono = bounds_mono,
              bounds_2_assumptions = bounds_2_assumptions
              ))
}
