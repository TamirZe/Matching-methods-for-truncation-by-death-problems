...
my_matching_func_pairmatch = function(X_sub_cols, m_data,
                            M=1, replace = FALSE, estimand = "ATC", 
                            mahal_match = 2, caliper, OBS_table){
  ...
  # TODO we can match wout caliper, just for the optimal weights property of paitmatch
  # TODO we can compare optimal match wout caliper from pairmatch to NN matching grom Match in Matching
  # in this function, caliper is probably in absolute value, and not in sd(principal_score)
  
  mhd <- match_on(f, data = m_data,  method = "mahalanobis") +
    caliper(match_on(principal_score, z = m_data$A),  0.05)
  
     pm2 <- pairmatch(mhd, data = m_data
                   # controls = M,  
                    ) 
     summary(pm2)
 #print(pairmatch(mhd, data = m_data, grouped = TRUE))
 # pm2 = na.omit(pm2)
  
  matched_data = data.table(m_data)
  matched_data$pm = pm2
  matched_data = na.omit(matched_data)
  matched_data = data.table(arrange(matched_data, pm, A))
  diff_each_pair = matched_data[, list(diff = diff(Y)), by = pm]
  ...
  # matched_data$pm = as.numeric(pm2)
  # matched_data = data.table(arrange(matched_data, pm, A))
  # diff_each_pair = matched_data[, list(diff = diff(Y)), by = pm]
  # SACE_matching_est_with_SO = mean(diff_each_pair$diff)
  ...
  matched_data = matched_data[, surv_pair := mean(S), by = pm]
  matched_data_S1 = data.table(filter(matched_data, surv_pair==1))
  diff_each_pair_S1 = matched_data_S1[, list(diff = diff(Y)), by = pm]
