...
#########################################################################################
# func: weighted SE with replacements for monotonicity
#func_est_wtd_var_matching_w_rep_mono = function(matched_data, f_alpha){ # f_alpha=1 gives regular weighted var
...
func_est_wtd_var_matching_w_rep = function(matched_data, f_alpha, SA_bool){ # f_alpha=1 gives regular weighted var
  M = nrow(filter(matched_data, A==0))
  est_var_Y0_mean = (1/M) * var(filter(matched_data, A==0)$Y) 
  est_var_Y1_unq = var(filter(matched_data, A==1)$Y) 
  if(SA_bool == "PPI"){
    sum_weights_squared_alpha = sum( ( f_alpha^2 ) * ( (filter(matched_data, A==1)$w)^2 ) ) 
...
SACE_estimation_1LEARNER_PPI = function(matched_data, reg_after_match, alpha1=1, 
  ...
  f_alpha_A1 = 1 / ( A1_S1_data$pi_tilde_as1 + ((1 - A1_S1_data$pi_tilde_as1) * alpha1) )
  f_alpha_A0 = 1 / ( A0_S1_data$pi_tilde_as1 + ((1 - A0_S1_data$pi_tilde_as1) * alpha1) )
  crude_Y1_adj = f_alpha_A1 * A1_S1_data$Y 
  crude_est_adj = mean(crude_Y1_adj) - mean(A0_S1_data$Y)
  crude_est_adj_se = sqrt( func_est_wtd_var_matching_w_rep(matched_data=matched_data, f_alpha=f_alpha_A1, SA_bool="PPI") )
  ...
  A0_S1_data$Y1_pred_adj = f_alpha_A0 * A0_S1_data$Y1_pred
  ...
  X_tilde = cbind(intercept = ( 1 - (1/f_alpha_A0) ) * X[,"intercept"], A = 1, 
        ( 1 - (1/f_alpha_A0) ) * X[,-which(colnames(X)=="intercept")]) 
  unit_diff_vec = f_alpha_A0 * (X_tilde %*% coeffs_regression_wout_inter)
  SACE_1LEARNER_adj_calc = mean(unit_diff_vec)
  print(SACE_1LEARNER_adj - SACE_1LEARNER_adj_calc)
  
  X_tilde_f_alpha = f_alpha_A0 * X_tilde
  sum_X_tilde_f_alpha = apply(X_tilde_f_alpha, 2, sum)
  var_sum_units_A0_S1_tilde_f_alpha = t(sum_X_tilde_f_alpha) %*%  vcov_beta_wout_inter %*% sum_X_tilde_f_alpha 
  SACE_1LEARNER_adj_se = sqrt( (1 / nrow(A0_S1_data)^2) * var_sum_units_A0_S1_tilde_f_alpha )
  # if(alpha1 != 1){ SACE_1LEARNER_adj_se = sqrt( (1 / nrow(A0_S1_data)^2) * var_sum_units_A0_S1_tilde_f_alpha )
  ...
  if(two_models_bool == FALSE){
    ...
    A0_S1_data$Y1_pred_inter_adj =  f_alpha_A0 * A0_S1_data$Y1_pred_inter
  ...
  if(two_models_bool == TRUE){
    ...
    sum_X = apply(X, 2, sum)
    sum_X_f_alpha = apply(X_f_alpha, 2, sum)
    
    var_sum_units_A0_S1 = ( t(sum_X_f_alpha) %*% vcov_beta_trt %*% sum_X_f_alpha ) + 
...
SACE_estimation_1LEARNER_mono = function(matched_data, reg_after_match, alpha0=1, xi=0, 
           coeffs_regression_one_model, coeffs_regression_two_models, two_models_bool=TRUE){
  ...
  A1_S1_data = filter(matched_data, A==1)
  A0_S1_data = filter(matched_data, A==0)
  f_alpha0 = (1 + xi) / (1 + (xi * alpha0)) 
  
  crude_Y0_adj = A0_S1_data$Y * f_alpha0
  crude_est_adj = mean(A1_S1_data$Y) - mean(crude_Y0_adj)
  crude_est_adj_se = sqrt( func_est_wtd_var_matching_w_rep(matched_data=matched_data, f_alpha=f_alpha0, SA_bool="mono") )
  ...
  sum_X_tilde_f_alpha0 = apply(X_tilde, 2, sum) # apply(X_tilde_g_alpha0, 2, sum)
  var_sum_units_A0_S1_tilde_f_alpha0 = t(sum_X_tilde_f_alpha0) %*%  vcov_beta_wout_inter %*% sum_X_tilde_f_alpha0 
  SACE_1LEARNER_adj_se = sqrt( (1 / nrow(A0_S1_data)^2) * var_sum_units_A0_S1_tilde_f_alpha0 )
  ...
  if(two_models_bool == TRUE){
    ...
    var_sum_units_A0_S1 = t(sum_X) %*%  vcov_beta_trt %*% sum_X + t(sum_X_adj) %*%  vcov_beta_untrt %*% sum_X_adj 
