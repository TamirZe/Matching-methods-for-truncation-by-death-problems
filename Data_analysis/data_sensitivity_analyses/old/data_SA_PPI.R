...
set.seed(101) 
matching_lst = matching_func_multiple_data(match_on = caliper_variable,
       cont_cov_mahal=covariates_mahal[-1], reg_cov=reg_after_match[-1], X_sub_cols=variables, 
...
set.seed(101) 
matching_lst_newWF = matching_all_measures_func(m_data=m_data, match_on=caliper_variable, 
                           covariates_mahal=covariates_mahal, reg_BC=reg_BC, X_sub_cols=variables, 
                           M=1, replace=TRUE, estimand="ATC", mahal_match=2, caliper=caliper)
data_pairs_lst = lapply(matching_lst_newWF[1:3], "[[", "matched_data")
data_pairs_lst_main = lapply(matching_datasets_lst[[2]][1:3], "[[", "matched_data")



summary(comparedf(data_pairs_lst_main$ps_lst %>% arrange(pair,A), data_pairs_lst_first_ver$data_pairs_only_ps %>% arrange(pair,A))) 
summary(comparedf(data_pairs_lst_main$mahal_lst %>% arrange(pair,A), data_pairs_lst_first_ver$data_pairs_maha_wout_cal %>% arrange(pair,A))) 
summary(comparedf(data_pairs_lst_main$mahal_cal_lst %>% arrange(pair,A), data_pairs_lst_first_ver$data_pairs_maha_cal_PS %>% arrange(pair,A))) 


View(matched_data_lst$PS)
View(reg_matched_lst$PS %>% arrange(pair,A))
View(data_pairs_lst$ps_lst %>% arrange(pair,A))


summary(comparedf(reg_matched_lst$PS %>% arrange(pair,A), data_pairs_lst$ps_lst %>% arrange(pair,A))) 
summary(comparedf(reg_matched_lst$Mahal %>% arrange(pair,A), data_pairs_lst$mahal_lst %>% arrange(pair,A))) 
summary(comparedf(reg_matched_lst$Mahal_PS_cal %>% arrange(pair,A), data_pairs_lst$mahal_cal_lst %>% arrange(pair,A))) 

matched_data_lst = list(PS = data_pairs_lst$ps_lst %>% distinct(id, .keep_all = TRUE) %>% subset(select = -pair),
                        Mahal = data_pairs_lst$mahal_lst %>% distinct(id, .keep_all = TRUE) %>% subset(select = -pair),
                        Mahal_PS_cal = data_pairs_lst$mahal_cal_lst %>% distinct(id, .keep_all = TRUE) %>% subset(select = -pair))
reg_matched_lst = data_pairs_lst
#########################################################################################
