...
function_my_EM = function(data, iterations = 12){
  dat_em = subset(data, select = c(id, X1, X2, g, g_num, prob.1, prob.2, prob.3, A, S, OBS))
  ...
  gamma_as_initial = c(0, 0)
  gamma_ns_initial = c(0, 0)
  ...
  for(i in 1:iterations){
    ...
    # TODO change here up to X6- intercept plus 5 x's.
    # TODO do it automatically so I dont have to write X1, X2...
    dat_em$p_as[dat_em$OBS == "O(1,1)"] = 
      1 / ( 1 + exp(-as.matrix(dat_em[dat_em$OBS == "O(1,1)", c("X1", "X2")]) %*% gamma_as_t) )
    dat_em$p_pro[dat_em$OBS == "O(1,1)"] = 
      1 / ( 1 + exp(as.matrix(dat_em[dat_em$OBS == "O(1,1)", c("X1", "X2")]) %*% gamma_as_t) )
    
    dat_em$p_ns[dat_em$OBS == "O(0,0)"] = 
      1 / ( 1 + exp(-as.matrix(dat_em[dat_em$OBS == "O(0,0)", c("X1", "X2")]) %*% gamma_ns_t) )
    ...
    positive_probs = apply(subset(dat_em, select = c(p_as, p_pro, p_ns)), 1, function(x) length(which(x>0)))
    ...
    dat_em_log = subset(dup_dat_em, select = c(X2, G_EM, w))
    ...
    ################################################################
    #library(nnet)
    mult_model = multinom(as.factor(G_EM_new) ~ X2, family = multinomial, data = dat_em_log,
