library(cowplot); library(ggpubr); library(AlphaPart); library(arsenal)

# write/read df wout misspecification
'''estimators_wout_misspec = small_large_pro
estimators_wout_misspec$shape = as.character(ifelse(estimators_wout_misspec$Replacements=="Wout", "24", "21"))
write.csv(estimators_wout_misspec,"C:/Users/tamir/Desktop/Estimators_wout_misspec.csv", row.names = FALSE)'''
Estimators_wout_misspec = read.csv("C:/Users/tamir/Desktop/Estimators_wout_misspec.csv", header = TRUE)
Estimators_wout_misspec$EstCombi = factor(Estimators_wout_misspec$EstCombi, levels = legend_levels)
#summary(comparedf(Estimators_wout_misspec, estimators_wout_misspec))

# write/read df with misspecification
'''estimators_with_misspec = small_large_pro
estimators_with_misspec$shape = as.character(ifelse(estimators_wout_misspec$Replacements=="Wout", "24", "21"))
write.csv(estimators_with_misspec,"C:/Users/tamir/Desktop/Estimators_with_misspec.csv", row.names = FALSE)'''
Estimators_with_misspec = read.csv("C:/Users/tamir/Desktop/Estimators_with_misspec.csv", header = TRUE)
Estimators_with_misspec$EstCombi = factor(Estimators_with_misspec$EstCombi, levels = legend_levels)
#summary(comparedf(Estimators_with_misspec, estimators_with_misspec))

# TODO select df without or with misspecification
current_df = Estimators_with_misspec # Estimators_wout_misspec # Estimators_with_misspec
