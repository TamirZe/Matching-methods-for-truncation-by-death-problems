...
summary(ps.pm)
psm_dist <- match_on(psm, data=nuclearplants)
caliper(psm.dist, 2)
ps.pm2 <- pairmatch(psm_dist, data = nuclearplants)
ps.pm3 <- pairmatch(psm_dist + caliper(psm.dist, 2), data = nuclearplants)
...
mhd2 <- match_on(pr ~ date + cap + scores(psm), data=nuclearplants, 
                 method = "mahalanobis")
