...
# for manual run of the large dataset estimators
...
###########################################################################################
seed= 222; gamma_pro = rep(0, dim_x); gamma_as = as.numeric(mat_gamma[1, c(1:dim_x)]);
gamma_ns =  as.numeric(mat_gamma[1, (dim_x+1): (2*dim_x)]); param_n = 100000;
...
large_dataset_estimators = fun_large_dataset_estimators(data_list, X_sub_cols, SACE, 
...
###########################################################################################
fun_large_dataset_estimators = function(data_list, X_sub_cols, SACE,
  ...
  colnames(large_matching_estimators) = paste0( "rep", rep(substr(replace_vec,1,1), each=6), "_",
  ...
  NOint_matching_reg_estimators = NOint_matching_reg_estimators[[1]]
  ...
  YESint_matching_reg_estimators = YESint_matching_reg_estimators[[1]]
...
# TODO don't change id before matching
# match_list_no_change_id_before_matching = list()
# for(j in c(1:length(replace_vec))){
#   print(j)
#   match_list_no_change_id_before_matching[[j]] =
#     lapply(1:length(data_list), function(l){
#       # what is the first argument?
#       my_matching_func_basic(X_sub_cols, data_list[[l]],
#                              weighting = FALSE, M=1, replace = replace_vec[j], estimand = "ATC", mahal_match = 2,
#                              min_PS = min_PS, min_diff_PS = min_diff_PS,
#                              caliper = caliper, OBS_table, change_id = FALSE)
#     })
# }
# 
# matching_estimators2 = lapply(1:length(replace_vec), function(j){
#   data.frame(t(unlist(list.rbind(lapply(match_list_no_change_id_before_matching[[j]],
#                                         head, 2)))))
# })
# matching_estimators2 = list.cbind(matching_estimators2)
# colnames(matching_estimators2) = paste0( "rep", rep(substr(replace_vec,1,1), each=6), "_",
#                                         paste0(rep(c("MATCH", "MATCH"), each = 3),
#                                                "_", c("all", "wout_O_0_0", "S1")),
#                                         rep(c(rep("", 3), rep("_HL", 3)), 2))
# 
# 
# # wout interactions
# NOint_matching_reg_estimators2 = lapply(2:2, function(j){
#   lapply(match_list_no_change_id_before_matching[[j]], "[[",
#          "NOinteractions_reg_adj_estimators_and_se")})
# NOint_matching_reg_estimators2 = NOint_matching_reg_estimators2[[1]]
# colnas = paste0(rep(c("all", "wout_O_0_0", "S1"), each=length(NOint_matching_reg_estimators2[[1]])), "_",
#                 rep(colnames(NOint_matching_reg_estimators2[[1]]), times=3))
# NOint_matching_reg_estimators2 = list.cbind(NOint_matching_reg_estimators2)
# colnames(NOint_matching_reg_estimators2) = colnas
# 
# # with interactions
# YESint_matching_reg_estimators2 = lapply(2:2, function(j){
#   lapply(match_list_no_change_id_before_matching[[j]], "[[",
#          "YESinteractions_reg_adj_estimators_and_se")})
# YESint_matching_reg_estimators2 = YESint_matching_reg_estimators2[[1]]
# colnas = paste0(rep(c("all", "wout_O_0_0", "S1"), each=length(YESint_matching_reg_estimators2[[1]])), "_",
#                 rep(colnames(YESint_matching_reg_estimators2[[1]]), times=3))
# YESint_matching_reg_estimators2 = list.cbind(YESint_matching_reg_estimators2)
# colnames(YESint_matching_reg_estimators2) = colnas
# 
# 
# large200_matching_estimators = rbind(matching_estimators, matching_estimators2)
# rownames(large200_matching_estimators) = c("change_id", "no_change_id")
# large200_matching_WLS_est = rbind(NOint_matching_reg_estimators, NOint_matching_reg_estimators2,
# YESint_matching_reg_estimators, YESint_matching_reg_estimators2)
# rownames(large200_matching_WLS_est) = c("NOint_change_id", "NOint_no_change_id",
#                                      "YESint_change_id", "YESint_no_change_id")
# large200_matching_WLS_est = large200_matching_WLS_est[,c(1:3,13:15,25:27)]
# 
# save(large200_matching_estimators, file = "large200_matching_estimators.RData")
# save(large200_matching_WLS_est, file = "large_matching_WLS_est.RData")
