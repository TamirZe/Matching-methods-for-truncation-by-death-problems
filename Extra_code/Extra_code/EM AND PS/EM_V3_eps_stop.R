...
# iterations = 12
#data = data_for_EM;  iterations = 12; epsilon = 0.001
#data = sg_data

function_my_EM = function(data, iterations = 12, epsilon = 0.001, my_sim = TRUE, dim_x){
  ...
  }else{dat_em = subset(data, select = c("id", X_sub_cols, "A", "S", "OBS"))}
  ...
  i = 1; convergence = FALSE
  while(!convergence & i <= iterations){
  ###for(i in 1:iterations){
    ...
    vec_diff = c(gamma_as_list[[i+1]] - gamma_as_list[[i]], gamma_ns_list[[i+1]] - gamma_ns_list[[i]])
    convergence = ifelse(max(abs(vec_diff)) < epsilon, TRUE, FALSE)
    print(paste0("max diff is ", max(abs(vec_diff))))
    # update i
    i = i + 1
  }
  ...
  return(list(dat_em = dat_em, last_coeff_as = last_coeff_as, last_coeff_ns = last_coeff_ns))
}
